...
disable() {
    systemctl disable x-ui
    if [[ $? == 0 ]]; then
        echo -e "${green}x-ui cancel the boot self-start successfully${plain}"
    else
        echo -e "${red}x-ui failed to cancel the boot self-start${plain}"
    fi
